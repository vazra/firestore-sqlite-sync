...
console.log('Loading module from ', __filename.slice(__dirname.length + 1))
...
export const updateWithSync = async (sync: ISync, collectionId: string, docId: string, data: IDoc) => {
  try {
    await sync.firedb
